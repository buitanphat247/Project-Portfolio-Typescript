...
export const categorizeRepositories = (repositories: GitHubRepo[]) => {
  ...
  repositories.forEach(repo => {
    ...
    if (name.includes('course') || name.includes('tutorial') || 
    ...
    // Kiểm tra nếu có từ "project"
      ...
      if (name.includes('client') || name.includes('frontend') || 
          description.includes('client') || description.includes('frontend') ||
          name.includes('ui') || name.includes('web')) {
        projects.client.push(repo);
      } else if (name.includes('server') || name.includes('backend') || 
                 description.includes('server') || description.includes('backend') ||
                 name.includes('api') || name.includes('db')) {
